...
public class TakeAttendance extends JFrame implements ChangeListener, ActionListener, ItemListener, KeyListener, MouseListener, MouseMotionListener, WindowListener, FocusListener, WindowFocusListener, MouseWheelListener, WindowStateListener{
	...
	public TakeAttendance(Subject sub, String date,Section sec,String myId){
		this.s = sub;
    	this.sub = sub;
    	this.date = date;
    	this.sec = sec;
    	this.myId = myId;
        setTitle("EdUBox Attendance ~ "+sub.getsubName()+" ~ "+date);
		
		this.setDefaultCloseOperation(TakeAttendance.DISPOSE_ON_CLOSE);
		this.setBounds(150,50,1130,720);
		this.setResizable(true);
		this.setTitle("AttendanceSheet ~ EdUBox ~ "+sub.getsubName()+" ~ "+date);
	...
	public TakeAttendance(Subject s,Section sec,String myId){
		this.s = s;
		this.sec = sec;
		this.myId = myId;
//		this.date ="today";
...
//        students.add(new Student("4", "Emily Davis", "emily@example.com", "A"));
        ...
        students = new AttendanceData().getAttendanceSheet(sub.getsId(),sub.getid(),date,sec.getid());
...
//        updateStudent(model,sub.getid(),sec.getid());
        

        // Populate the table with student data
        for (Student student : students) {
            Object[] row = {student.getstdId(), student.getName(), student.getEmail(), student.getAttendanceStatus()};
            model.addRow(row);
        }
        ...
        submitButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                ArrayList<Student> attendanceStd = new ArrayList<>();
                AttendanceData attData = new AttendanceData();
                attendanceStd = attData.takeAttendance(attendanceData,sub,date,sec,myId);
                
                
                // For now, let's just display the collected data in a message dialog
                StringBuilder message = new StringBuilder();
                for (Student student : attendanceStd) {
                    message.append("ID: ").append(student.getstdId()).append("~  \t");
                ...
                JOptionPane.showMessageDialog(TakeAttendance.this, message.toString());
            }
        });
        ...
        buttonPanel.setBounds(280, 85, 445, 35);
...
//		JOptionPane.showMessageDialog(null, " name "+s+" rand  "+mm+" uuid ");
	...
	public static void main(String[] args) {
		TakeAttendance frm = new TakeAttendance();
		frm.setVisible(true);
