...
//			System.out.println(""+s.getProfilePicInBytes()+" stdudentData "+s.getImgData());
			...
			JOptionPane.showMessageDialog(null, "Department Data did not add, Try Again!"+e1);
			e1.printStackTrace();
		}finally {
			try {
				pst.close();
...
//				res.close();
	...
	public int updateDep(schoolDep d) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Department Data not updated, Try Again!"+e1);
			e1.printStackTrace();
		}finally {
			try {
				pst.close();
	...
	public String depId(int id) {
		...
		try {
			...
			if(rs.next()) {
				...
				JOptionPane.showMessageDialog(null, "Department Not Found, Try Again!");
			}
